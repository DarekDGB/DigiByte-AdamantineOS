...
def _assert_required_present(obj: dict, required: Iterable[str], *, where: str) -> None:
...
def _validate_request_v2(req: dict) -> None:
    # Guard: if a request fixture accidentally contains a response, fail with a clear message.
    if "status" in req and "decision" in req and req.get("v") == "execution_response_v2":
        raise AssertionError(
            "This fixture is a RESPONSE but is stored under request_*.json. "
            "Fix the fixture pack: request_*.json must contain execution_request_v2."
        )

    _assert_optional_allowlist(
        req,
        ["v", "request_id", "intent", "context", "authority", "timebox", "nonce", "payload", "audit"],
        where="request",
    )
    _assert_required_present(
        req,
        ["v", "request_id", "intent", "context", "authority", "timebox", "nonce", "payload"],
        where="request",
    )
    ...
    _assert_required_present(auth, ["class", "scope"], where="request.authority")
    _assert_str(auth["class"], where="request.authority.class")
    ...
    _assert_required_present(tb, ["issued_at", "expires_at"], where="request.timebox")
    _assert_str(tb["issued_at"], where="request.timebox.issued_at")
    _assert_str(tb["expires_at"], where="request.timebox.expires_at")
    ...
    _assert_optional_allowlist(nonce, ["value", "store", "mode"], where="request.nonce")
    _assert_required_present(nonce, ["value", "store", "mode"], where="request.nonce")
    ...
    # payload (strict wiring)
    payload = _assert_dict(req["payload"], where="request.payload")
    _assert_optional_allowlist(payload, ["evidence", "body"], where="request.payload")
    _assert_required_present(payload, ["evidence", "body"], where="request.payload")

    evidence = _assert_dict(payload["evidence"], where="request.payload.evidence")
    _assert_optional_allowlist(evidence, ["qid", "oracle", "shield"], where="request.payload.evidence")
    _assert_required_present(evidence, ["qid", "oracle", "shield"], where="request.payload.evidence")
...
def _validate_response_v2(resp: dict) -> None:
    ...
    _assert_required_present(resp, ["v", "request_id", "status", "reason_id", "context_hash", "decision"], where="response")
    ...
    _assert_optional_allowlist(gates, ["tva", "eqc", "wsqk"], where="response.decision.gates")
    _assert_required_present(gates, ["tva", "eqc", "wsqk"], where="response.decision.gates")
    for gate in ("tva", "eqc", "wsqk"):
        g = _assert_dict(gates[gate], where=f"response.decision.gates.{gate}")
        _assert_optional_allowlist(g, ["allowed", "reason_id"], where=f"response.decision.gates.{gate}")
        _assert_required_present(g, ["allowed", "reason_id"], where=f"response.decision.gates.{gate}")
    ...
    _assert_optional_allowlist(tb, ["valid", "issued_at", "expires_at", "max_skew_seconds", "reason_id"], where="response.decision.timebox")
    _assert_required_present(tb, ["valid", "issued_at", "expires_at", "max_skew_seconds", "reason_id"], where="response.decision.timebox")
    ...
    _assert_optional_allowlist(nonce, ["consumed", "store", "value", "reason_id"], where="response.decision.nonce")
    _assert_required_present(nonce, ["consumed", "store", "value", "reason_id"], where="response.decision.nonce")
    ...
    _assert_optional_allowlist(ev, ["qid", "shield", "oracle"], where="response.decision.evidence")
    _assert_required_present(ev, ["qid", "shield", "oracle"], where="response.decision.evidence")
    for name in ("qid", "shield", "oracle"):
        e = _assert_dict(ev[name], where=f"response.decision.evidence.{name}")
        _assert_optional_allowlist(e, ["present", "valid", "reason_id"], where=f"response.decision.evidence.{name}")
        _assert_required_present(e, ["present", "valid", "reason_id"], where=f"response.decision.evidence.{name}")
    ...
    _assert_optional_allowlist(pol, ["mode", "override_allowed", "reason_id"], where="response.decision.policy")
    _assert_required_present(pol, ["mode", "override_allowed", "reason_id"], where="response.decision.policy")
